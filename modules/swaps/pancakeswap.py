...
class PancakeSwap(Web3Swapper):
    ...
    def __init__(
        ...
        self.contract_quoter = self.acc.w3.eth.contract(
            address=eth_utils.address.to_checksum_address(
                config.PANCAKESWAP.CONTRACTS.get(
                    self.acc.network.get(NETWORK_FIELDS.NAME)
                ).get("quoter")
            ),
            abi=config.PANCAKESWAP.ABI_QUOTER,
        )
        self.contract_router = self.acc.w3.eth.contract(
            ...
            abi=config.PANCAKESWAP.ABI_ROUTER,
        )
    ...
    async def _get_pool_fee(self, from_token: Token_Info, to_token: Token_Info):
        ...
        fees = config.PANCAKESWAP.CONTRACTS.get(
            self.acc.network.get(NETWORK_FIELDS.NAME)
        ).get("pairs")
        fee = None
        for pair in pairs:
    ...
    async def _perform_swap(
        ...
        if not amount_in:
            logger.error(f"FAIL GET AMOUNT_IN")
            return RESULT_TRANSACTION.FAIL
        amount_in = Token_Amount(amount=amount_in, decimals=to_token.decimals, wei=True)
        recipient = (
            self.acc.address
            if not await Token_Info.is_native_token(
                network=self.acc.network, token=to_token
            )
            else eth_utils.address.to_checksum_address(
                "0x0000000000000000000000000000000000000002"
            )
        )
        data_swap = await self.get_data(
            contract=self.contract_router,
            function_of_contract="exactInput",
            args=[
                (
                    path,
                    recipient,
        ...
        if data_swap is None:
            logger.error(f"FAIL GET TX_DATA")
            return RESULT_TRANSACTION.FAIL
